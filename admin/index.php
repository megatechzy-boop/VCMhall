    ...
    if ($action === 'login' && !$loggedIn) {
        ...
        try {
            if ($action === 'block') {
                ...
                if (!booking_valid_date($date)) admin_finish('Choose a valid current or future date.');
                $insert = $db->prepare("INSERT INTO bookings (name, phone, event_type, event_date, booked_date, message, status, record_type, source) VALUES ('Admin block', '', 'Manual block', ?, ?, '', 'blocked', 'block', 'other')");
                $insert->execute([$date, $date]); $id = (int) $db->lastInsertId();
                admin_activity($db, $id, 'date_blocked', 'Date blocked by admin'); admin_finish('Date blocked.');
            }
            ...
            if ($action === 'add_booking') {
                ...
                if ($name === '' || admin_text_length($name) > 120 || !preg_match('/^[0-9+() -]{10,18}$/', $phone)
                    ...
                    || !is_numeric($total) || (float) $total < 0 || (float) $total > 99999999) admin_finish('Check the booking details and try again.');
                $insert = $db->prepare("INSERT INTO bookings (name, phone, email, event_type, event_date, booked_date, guests, message, status, record_type, source, total_amount, confirmed_at) VALUES (?, ?, ?, ?, ?, ?, ?, '', 'confirmed', 'booking', ?, ?, CURRENT_TIMESTAMP)");
                $insert->execute([$name, $phone, $email, $event, $date, $date, $guests === '' ? null : (int) $guests, $source, (float) $total]);
            ...
            admin_finish($error->getCode() === '23000' ? 'That date is already booked or blocked.' : 'The change could not be saved.');
        } catch (Throwable $error) {
...
if ($loggedIn) {
    ...
    if ($schemaReady) {
        ...
        $metrics = admin_metrics($db); $calendarDays = admin_calendar($db, $month); $related = admin_related($db, $rows);
    }
}
...
<?php if (!$schemaReady): ?><section class="migration-card"><h2>Database update required</h2><p>Run <code>migrations/20260919_admin_dashboard.sql</code> once on the existing database. Existing records are preserved.</p></section>
<?php else: ?>
